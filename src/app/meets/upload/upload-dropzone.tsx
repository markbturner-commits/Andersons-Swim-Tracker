...
// Drag-and-drop PDF dropzone + in-session upload queue.
...
interface Props {
  // uploadIds already shown in the server-rendered "Awaiting confirmation"
  // panel above the dropzone. Items whose uploadId matches are hidden here so
  // the same row doesn't appear twice while the user stays on the page.
  knownPendingUploadIds: string[];
}
...
export function UploadDropzone({ knownPendingUploadIds }: Props) {
  ...
  const knownSet = useMemo(
    () => new Set(knownPendingUploadIds),
    [knownPendingUploadIds],
  );
  ...
  const processQueue = useCallback(
    async (queued: UploadItem[], opts: { overwrite?: boolean } = {}) => {
      ...
      // Multi-file: refresh so the server-rendered "Awaiting confirmation"
      // panel picks up the new rows on next render.
      if (successCount > 0) {
        router.refresh();
      }
    },
  ...
  // Hide items whose uploadId is already shown in the server-rendered panel
  // above — happens after router.refresh() once the parse-pdf insert lands.
  const visibleItems = items.filter(
    (i) => !(i.uploadId && knownSet.has(i.uploadId)),
  );
  const successItems = visibleItems.filter((i) => i.status === "success");
  const showQueueUI = visibleItems.length > 0;
        ...
        <section className="mt-6">
          <div className="flex items-center justify-between">
            <h2 className="font-display text-lg text-navy">
              This session ({visibleItems.length})
            </h2>
            {!running && (
              <button
                type="button"
